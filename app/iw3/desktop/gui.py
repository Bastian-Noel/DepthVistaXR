...
from .openxr_output import detect_openxr_runtime

CONFIG_DIR = ensure_home_dir(
    "iw3", path.join(path.dirname(__file__), "..", "..", "tmp")
)
...
HAS_WINDOWS_CAPTURE_CUDA = (
    bool(importlib.util.find_spec("wc_cuda"))
...
myEVT_OPENXR_STATUS = wx.NewEventType()
EVT_OPENXR_STATUS = wx.PyEventBinder(myEVT_OPENXR_STATUS, 0)
...
class OpenXRStatusEvent(wx.PyCommandEvent):
    def __init__(self, state, detail):
        super().__init__(myEVT_OPENXR_STATUS, -1)
        self.state = state
        self.detail = detail


class OpenXRStatusGUI:
    def __init__(self, parent):
        self.parent = parent

    def __call__(self, state, detail):
        wx.PostEvent(self.parent, OpenXRStatusEvent(state, detail))
...
class MainFrame(wx.Frame):
    ...
    def initialize_component(self):
        ...
        # visual capture source selector
        self.grp_source = wx.StaticBox(self.pnl_options, label=T("Capture Source"))
        self.lbl_source_type = wx.StaticText(
            self.grp_source, label=T("What do you want to capture?")
        )
        self.cbo_source_type = wx.ComboBox(
            self.grp_source,
            choices=[T("Entire Screen"), T("Application Window")],
            name="cbo_source_type",
        )
        self.cbo_source_type.SetEditable(False)
        self.cbo_source_type.SetSelection(0)
        self.lbl_source_choice = wx.StaticText(self.grp_source, label=T("Screen"))
        self.cbo_source_choice = wx.ComboBox(self.grp_source, name="cbo_source_choice")
        self.cbo_source_choice.SetEditable(False)
        self.btn_source_refresh = wx.Button(self.grp_source, label=T("Refresh Preview"))
        self.lbl_source_help = wx.StaticText(
            self.grp_source,
            label=T(
                "Choose the screen or application that will appear in the headset."
            ),
        )
        self.lbl_source_help.Wrap(self.FromDIP(640))
        preview_width, preview_height = self.FromDIP((320, 180))
        self.source_preview_size = (preview_width, preview_height)
        self.bmp_source_preview = wx.StaticBitmap(
            self.grp_source,
            bitmap=wx.Bitmap(preview_width, preview_height),
            size=(preview_width, preview_height),
            name="bmp_source_preview",
        )
        self.bmp_source_preview.SetMinSize(self.source_preview_size)
        self.bmp_source_preview.SetMaxSize(self.source_preview_size)
        source_controls = wx.BoxSizer(wx.VERTICAL)
        source_row = wx.BoxSizer(wx.HORIZONTAL)
        source_row.Add(
            self.lbl_source_type, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=6
        )
        source_row.Add(self.cbo_source_type, 1, flag=wx.RIGHT, border=8)
        source_row.Add(
            self.lbl_source_choice, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=6
        )
        source_row.Add(self.cbo_source_choice, 2, flag=wx.RIGHT, border=8)
        source_row.Add(self.btn_source_refresh)
        source_controls.Add(source_row, flag=wx.EXPAND | wx.BOTTOM, border=5)
        source_controls.Add(self.lbl_source_help, flag=wx.EXPAND | wx.BOTTOM, border=5)
        source_controls.Add(self.bmp_source_preview, flag=wx.ALIGN_CENTER_HORIZONTAL)
        sizer_source = wx.StaticBoxSizer(self.grp_source, wx.VERTICAL)
        sizer_source.Add(source_controls, 1, wx.ALL | wx.EXPAND, 6)
        ...
        self.lbl_simple_profile = wx.StaticText(
            self.grp_stereo, label=T("Usage Profile")
        )
        self.cbo_simple_profile = wx.ComboBox(
            self.grp_stereo,
            choices=[T("Smooth"), T("Balanced"), T("Best Quality")],
            name="cbo_simple_profile",
        )
        self.cbo_simple_profile.SetEditable(False)
        self.cbo_simple_profile.SetSelection(1)
        self.lbl_simple_profile_help = wx.StaticText(
            self.grp_stereo, label=T("Balanced is recommended for real-time OpenXR.")
        )

        self.lbl_divergence = wx.StaticText(self.grp_stereo, label=T("3D Strength"))
        self.cbo_divergence = EditableComboBox(
            self.grp_stereo,
            choices=["5.0", "4.0", "3.0", "2.5", "2.0", "1.0"],
            name="cbo_divergence",
        )
        ...
        self.lbl_convergence = wx.StaticText(
            self.grp_stereo, label=T("Convergence Plane")
        )
        ...
        self.lbl_synthetic_view = wx.StaticText(
            self.grp_stereo, label=T("Synthetic View")
        )
        ...
        self.chk_small_model_only = wx.CheckBox(
            self.grp_stereo,
            label=T("List small model only"),
            name="chk_small_model_only",
        )
        ...
        self.cbo_depth_model = wx.ComboBox(
            self.grp_stereo, choices=depth_models, name="cbo_depth_model"
        )
        self.cbo_depth_model.SetEditable(False)
        self.cbo_depth_model.SetSelection(depth_models.index("Any_V2_S"))

        self.lbl_resolution = wx.StaticText(
            self.grp_stereo, label=T("Depth") + " " + T("Resolution")
        )
        self.cbo_resolution = EditableComboBox(
            self.grp_stereo, choices=["Default", "512"], name="cbo_zoed_resolution"
        )
        self.cbo_resolution.SetSelection(0)

        self.lbl_foreground_scale = wx.StaticText(
            self.grp_stereo, label=T("Foreground Scale")
        )
        ...
        self.lbl_edge_dilation = wx.StaticText(
            self.grp_stereo, label=T("Edge Fix"), name="lbl_edge_dilation"
        )
        ...
        self.lbl_edge_dilation.SetToolTip(
            T("Reduce distortion of foreground and background edges")
        )
        self.cbo_edge_dilation.SetToolTip(T("X or XY"))
        self.cbo_edge_dilation_y.SetToolTip(T("Y"))

        self.chk_ema_normalize = wx.CheckBox(
            self.grp_stereo, label=T("Flicker Reduction"), name="chk_ema_normalize"
        )
        ...
        self.cbo_ema_decay = EditableComboBox(
            self.grp_stereo,
            choices=["0.99", "0.95", "0.9", "0.75", "0.5"],
            name="cbo_ema_decay",
        )
        ...
        self.chk_preserve_screen_border.SetToolTip(
            T("Force set screen border parallax to zero")
        )

        self.lbl_stereo_format = wx.StaticText(
            self.grp_stereo, label=T("Stereo Format")
        )
        ...
        self.lbl_anaglyph_method = wx.StaticText(
            self.grp_stereo, label=T("Anaglyph Method")
        )
        ...
        i = 0
        layout.Add(
            self.lbl_simple_profile, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_simple_profile, (i, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(
            self.lbl_simple_profile_help, (i := i + 1, 0), (1, 3), flag=wx.EXPAND
        )
        ...
        layout.Add(
            self.lbl_synthetic_view, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        ...
        layout.Add(
            self.lbl_foreground_scale, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_foreground_scale, (i, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(
            self.lbl_edge_dilation, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_edge_dilation, (i, 1), flag=wx.EXPAND)
        layout.Add(self.cbo_edge_dilation_y, (i, 2), flag=wx.EXPAND)
        layout.Add(
            self.chk_ema_normalize, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_ema_decay, (i, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(
            self.chk_preserve_screen_border,
            (i := i + 1, 0),
            (0, 1),
            flag=wx.ALIGN_CENTER_VERTICAL,
        )
        layout.Add(
            self.lbl_stereo_format, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_stereo_format, (i, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(
            self.lbl_anaglyph_method, (i := i + 1, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_anaglyph_method, (i, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(
            self.lbl_format_device,
            (i := i + 1, 1),
            (1, 2),
            flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL,
        )
        ...
        self.cbo_view_mode = wx.ComboBox(
            self.grp_network, choices=[T("SBS Preview Window")], name="cbo_view_mode"
        )
        self.cbo_view_mode.SetEditable(False)
        self.cbo_view_mode.SetSelection(0)

        self.chk_bind_addr = wx.CheckBox(
            self.grp_network, label=T("Address"), name="chk_bind_addr"
        )
        self.txt_bind_addr = IpAddrCtrl(
            self.grp_network, size=self.FromDIP((200, -1)), name="txt_bind_addr"
        )
        self.chk_bind_addr.SetValue(False)
        self.txt_bind_addr.SetValue("127.0.0.1")
        self.btn_detect_ip = GenBitmapButton(
            self.grp_network, bitmap=load_icon("view-refresh.png")
        )
        ...
        self.cbo_stream_fps = EditableComboBox(
            self.grp_network,
            choices=["60", "30", "24", "15", "8"],
            name="cbo_stream_fps",
        )
        self.cbo_stream_fps.SetSelection(1)

        self.chk_uncap_fps = wx.CheckBox(
            self.grp_network, label=T("Uncap FPS"), name="chk_uncap_fps"
        )
        self.chk_uncap_fps.SetValue(False)

        self.lbl_network_stream_height = wx.StaticText(
            self.grp_network, label=T("Streaming Resolution")
        )
        self.cbo_network_stream_height = EditableComboBox(
            self.grp_network,
            choices=["1080", "900", "720"],
            name="cbo_network_stream_height",
        )
        self.cbo_network_stream_height.SetSelection(0)

        self.lbl_stream_quality = wx.StaticText(
            self.grp_network, label=T("MJPEG Quality")
        )
        self.cbo_stream_quality = EditableComboBox(
            self.grp_network,
            choices=["100", "95", "90", "85", "80"],
            name="cbo_stream_quality",
        )
        self.cbo_stream_quality.SetSelection(2)
        self.chk_pad_16_9 = wx.CheckBox(
            self.grp_network, label=T("Padding") + " 16:9", name="chk_pad_16_9"
        )
        self.chk_pad_16_9.SetValue(False)

        self.chk_gpu_jpeg = wx.CheckBox(
            self.grp_network, label=T("GPU JPEG"), name="chk_gpu_jpeg"
        )
        ...
        self.chk_auth = wx.CheckBox(
            self.grp_network, label=T("Basic Authentication"), name="chk_auth"
        )
        ...
        self.txt_auth_password = wx.TextCtrl(
            self.grp_network, style=wx.TE_PASSWORD, name="txt_auth_password"
        )
        ...
        layout.Add(
            self.lbl_bind_addr_warning, (2, 0), (0, 3), flag=wx.ALIGN_CENTER_VERTICAL
        )
        ...
        layout.Add(
            self.lbl_network_stream_height, (6, 0), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.cbo_network_stream_height, (6, 1), flag=wx.EXPAND)
        ...
        # OpenXR settings
        self.grp_openxr = wx.StaticBox(self.pnl_options, label="OpenXR")
        self.chk_openxr = wx.CheckBox(
            self.grp_openxr, label=T("Enable OpenXR"), name="chk_openxr"
        )
        self.btn_openxr_toggle = wx.Button(
            self.grp_openxr, label=T("Start"), name="btn_openxr_toggle"
        )
        runtime = detect_openxr_runtime()
        self.lbl_openxr_runtime = wx.StaticText(
            self.grp_openxr, label=T("Detected Runtime")
        )
        self.txt_openxr_runtime = wx.TextCtrl(
            self.grp_openxr,
            value=runtime["runtime"],
            style=wx.TE_READONLY,
            name="txt_openxr_runtime",
        )
        self.lbl_openxr_state = wx.StaticText(
            self.grp_openxr, label=T("Headset / Session")
        )
        self.txt_openxr_state = wx.TextCtrl(
            self.grp_openxr,
            value=T("Stopped"),
            style=wx.TE_READONLY,
            name="txt_openxr_state",
        )
        self.lbl_openxr_controller = wx.StaticText(
            self.grp_openxr, label=T("Controllers")
        )
        self.txt_openxr_controller = wx.TextCtrl(
            self.grp_openxr,
            value=T("Waiting"),
            style=wx.TE_READONLY,
            name="txt_openxr_controller",
        )
        self.lbl_openxr_controller_help = wx.StaticText(
            self.grp_openxr,
            label=T(
                "Both sticks: left/right seek video (Left/Right arrows) • up/down changes distance • Hold grip: up/down changes size and left/right changes curvature • A/X or stick click: Space/Pause • B/Y: stop OpenXR"
            ),
        )
        self.lbl_openxr_controller_help.Wrap(self.FromDIP(430))
        self.lbl_openxr_projection = wx.StaticText(
            self.grp_openxr, label=T("Projection")
        )
        self.cbo_openxr_projection = wx.ComboBox(
            self.grp_openxr, choices=["Curved", "Flat"], name="cbo_openxr_projection"
        )
        self.cbo_openxr_projection.SetEditable(False)
        self.cbo_openxr_projection.SetSelection(0)
        self.lbl_stream_height = wx.StaticText(
            self.grp_openxr, label=T("Streaming Resolution")
        )
        self.cbo_stream_height = EditableComboBox(
            self.grp_openxr,
            choices=["1080", "900", "720"],
            name="cbo_stream_height",
        )
        self.cbo_stream_height.SetSelection(0)
        self.lbl_openxr_distance = wx.StaticText(
            self.grp_openxr, label=T("Screen Distance")
        )
        self.sld_openxr_distance = wx.Slider(
            self.grp_openxr,
            value=20,
            minValue=5,
            maxValue=100,
            style=wx.SL_HORIZONTAL,
            name="sld_openxr_distance",
        )
        self.txt_openxr_distance_value = wx.StaticText(self.grp_openxr, label="2.0 m")
        self.lbl_openxr_width = wx.StaticText(self.grp_openxr, label=T("Screen Size"))
        self.sld_openxr_width = wx.Slider(
            self.grp_openxr,
            value=30,
            minValue=5,
            maxValue=100,
            style=wx.SL_HORIZONTAL,
            name="sld_openxr_width",
        )
        self.txt_openxr_width_value = wx.StaticText(self.grp_openxr, label="3.0 m")
        self.lbl_openxr_height = wx.StaticText(
            self.grp_openxr, label=T("Height (m, 0=auto)")
        )
        self.sld_openxr_height = wx.SpinCtrlDouble(
            self.grp_openxr,
            value="0.0",
            min=0.0,
            max=20.0,
            inc=0.1,
            name="sld_openxr_height",
        )
        self.lbl_openxr_curvature = wx.StaticText(
            self.grp_openxr, label=T("Screen Curvature")
        )
        self.sld_openxr_curvature = wx.Slider(
            self.grp_openxr,
            value=30,
            minValue=0,
            maxValue=120,
            style=wx.SL_HORIZONTAL,
            name="sld_openxr_curvature",
        )
        self.txt_openxr_curvature_value = wx.StaticText(self.grp_openxr, label="30°")
        self.btn_openxr_recenter = wx.Button(self.grp_openxr, label=T("Recenter View"))
        self.chk_openxr_show_fps = wx.CheckBox(
            self.grp_openxr, label=T("Show FPS"), name="chk_openxr_show_fps"
        )
        self.chk_openxr_show_fps.SetValue(False)
        self.txt_openxr_log = wx.TextCtrl(
            self.grp_openxr,
            style=wx.TE_MULTILINE | wx.TE_READONLY,
            size=self.FromDIP((300, 55)),
            name="txt_openxr_log",
        )
        if not runtime["available"]:
            self.chk_openxr.Disable()
            self.btn_openxr_toggle.Disable()
            self.txt_openxr_log.SetValue(T("No active OpenXR runtime detected"))

        layout = wx.GridBagSizer(vgap=5, hgap=4)
        layout.Add(self.chk_openxr, (0, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.btn_openxr_toggle, (0, 1), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_runtime, (1, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.txt_openxr_runtime, (1, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_state, (2, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.txt_openxr_state, (2, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_controller, (3, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.txt_openxr_controller, (3, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_controller_help, (4, 0), (1, 3), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_projection, (5, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.cbo_openxr_projection, (5, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(self.lbl_stream_height, (6, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.cbo_stream_height, (6, 1), (1, 2), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_distance, (7, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.sld_openxr_distance, (7, 1), flag=wx.EXPAND)
        layout.Add(
            self.txt_openxr_distance_value, (7, 2), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.lbl_openxr_width, (8, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.sld_openxr_width, (8, 1), flag=wx.EXPAND)
        layout.Add(self.txt_openxr_width_value, (8, 2), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.lbl_openxr_height, (9, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.sld_openxr_height, (9, 1), flag=wx.EXPAND)
        layout.Add(self.lbl_openxr_curvature, (10, 0), flag=wx.ALIGN_CENTER_VERTICAL)
        layout.Add(self.sld_openxr_curvature, (10, 1), flag=wx.EXPAND)
        layout.Add(
            self.txt_openxr_curvature_value, (10, 2), flag=wx.ALIGN_CENTER_VERTICAL
        )
        layout.Add(self.btn_openxr_recenter, (11, 0), (1, 3), flag=wx.EXPAND)
        layout.Add(self.chk_openxr_show_fps, (12, 0), (1, 3), flag=wx.EXPAND)
        layout.Add(self.txt_openxr_log, (13, 0), (1, 3), flag=wx.EXPAND)
        layout.AddGrowableCol(1, 1)
        sizer_openxr = wx.StaticBoxSizer(self.grp_openxr, wx.VERTICAL)
        sizer_openxr.Add(layout, 1, wx.ALL | wx.EXPAND, 4)
        ...
        self.cbo_device = wx.ComboBox(
            self.grp_processor, size=self.FromDIP((200, -1)), name="cbo_device"
        )
        ...
        self.cbo_screenshot = wx.ComboBox(
            self.grp_processor, choices=screenshot_backends, name="cbo_screenshot"
        )
        ...
        self.lbl_monitor_index = wx.StaticText(
            self.grp_processor, label=T("Monitor Index")
        )
        ...
        self.btn_reload_window_name = GenBitmapButton(
            self.grp_processor, bitmap=load_icon("view-refresh.png")
        )
        self.btn_reload_window_name.SetToolTip(T("Reload"))

        self.lbl_crop_region = wx.StaticText(
            self.grp_processor, label=T("Crop Region (px)")
        )

        self.lbl_crop_top = wx.StaticText(self.grp_processor, label=T("Top"))
        self.txt_crop_top = IntCtrl(
            self.grp_processor,
            size=self.FromDIP((60, -1)),
            allow_none=False,
            min=0,
            max=500,
            name="txt_crop_top",
        ...
        self.chk_compile = wx.CheckBox(
            self.grp_processor, label=T("torch.compile"), name="chk_compile"
        )
        ...
        self.lbl_adj_divergence = wx.StaticText(
            self.grp_adjustment, label=T("3D Strength")
        )
        self.sld_adj_divergence = wx.SpinCtrlDouble(
            ...
            min=0.0,
            max=10.0,
            inc=DIVERGENCE_INC_DEFAULT,
        )
        self.sld_adj_divergence.SetDigits(2)
        self.lbl_adj_convergence = wx.StaticText(
            self.grp_adjustment, label=T("Convergence Plane")
        )
        self.sld_adj_convergence = wx.SpinCtrlDouble(
            ...
            min=0.0,
            max=1.0,
            inc=CONVERGENCE_INC_DEFAULT,
        )
        self.lbl_adj_foreground_scale = wx.StaticText(
            self.grp_adjustment, label=T("Foreground Scale")
        )
        self.sld_adj_foreground_scale = wx.SpinCtrlDouble(
            ...
            min=-3.0,
            max=3.0,
            inc=FOREGROUNDSCALE_INC_DEFAULT,
        )
        self.lbl_adj_edge_dilation = wx.StaticText(
            self.grp_adjustment, label=T("Edge Fix")
        )
        ...
        self.tgl_adj_fine = wx.ToggleButton(
            self.grp_adjustment, label=T("Fine Adjustment"), name="tgl_adj_fine"
        )
        ...
        layout.SetEmptyCellSize((0, 0))
        layout.Add(sizer_source, (0, 0), (1, 2), flag=wx.ALL | wx.EXPAND, border=4)
        layout.Add(sizer_adjustment, (1, 0), (0, 1), flag=wx.ALL | wx.EXPAND, border=4)
        layout.Add(sizer_stereo, (2, 0), (2, 0), flag=wx.ALL | wx.EXPAND, border=4)
        layout.Add(sizer_network, (2, 1), flag=wx.ALL | wx.EXPAND, border=4)
        layout.Add(sizer_processor, (3, 1), flag=wx.ALL | wx.EXPAND, border=4)
        layout.Add(sizer_openxr, (4, 0), (1, 2), flag=wx.ALL | wx.EXPAND, border=4)
        ...
        self.txt_url = wx.TextCtrl(
            self.pnl_process, size=self.FromDIP((300, -1)), style=wx.TE_READONLY
        )
        self.btn_url = GenBitmapButton(
            self.pnl_process, bitmap=load_icon("go-next.png")
        )
        ...
        self.chk_advanced_mode = wx.CheckBox(
            self.pnl_preset, label=T("Advanced Mode"), name="chk_advanced_mode"
        )
        self.chk_advanced_mode.SetValue(False)

        layout = wx.BoxSizer(wx.HORIZONTAL)
        layout.Add(
            self.lbl_language, flag=wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT, border=2
        )
        layout.Add(self.cbo_language, flag=wx.ALL, border=2)
        layout.AddSpacer(8)
        layout.Add(
            self.chk_advanced_mode, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=2
        )
        ...
        self.chk_advanced_mode.Bind(wx.EVT_CHECKBOX, self.update_ui_mode)
        self.cbo_simple_profile.Bind(wx.EVT_TEXT, self.on_simple_profile)
        self.cbo_source_type.Bind(wx.EVT_TEXT, self.update_source_selector)
        self.cbo_source_choice.Bind(wx.EVT_TEXT, self.on_source_choice)
        self.btn_source_refresh.Bind(wx.EVT_BUTTON, self.update_source_preview)
        ...
        self.lbl_divergence_warning.Bind(
            wx.EVT_LEFT_DOWN, self.on_click_divergence_warning
        )
        ...
        self.lbl_bind_addr_warning.Bind(
            wx.EVT_LEFT_DOWN, self.on_click_bind_addr_warning
        )
        ...
        self.sld_adj_convergence.Bind(
            wx.EVT_SPINCTRLDOUBLE, self.update_args_adjustment
        )
        ...
        self.sld_adj_foreground_scale.Bind(
            wx.EVT_SPINCTRLDOUBLE, self.update_args_adjustment
        )

        self.cbo_view_mode.Bind(wx.EVT_TEXT, self.on_text_changed_cbo_view_mode)
        self.chk_openxr.Bind(wx.EVT_CHECKBOX, self.update_openxr_state)
        self.btn_openxr_toggle.Bind(wx.EVT_BUTTON, self.on_click_openxr_toggle)
        self.btn_openxr_recenter.Bind(wx.EVT_BUTTON, self.on_click_openxr_recenter)
        self.cbo_openxr_projection.Bind(wx.EVT_TEXT, self.update_openxr_settings)
        self.sld_openxr_distance.Bind(wx.EVT_SLIDER, self.update_openxr_settings)
        self.sld_openxr_width.Bind(wx.EVT_SLIDER, self.update_openxr_settings)
        self.sld_openxr_height.Bind(wx.EVT_SPINCTRLDOUBLE, self.update_openxr_settings)
        self.sld_openxr_curvature.Bind(wx.EVT_SLIDER, self.update_openxr_settings)
        self.chk_openxr_show_fps.Bind(wx.EVT_CHECKBOX, self.update_openxr_settings)
        ...
        self.Bind(EVT_OPENXR_STATUS, self.on_openxr_status)
        ...
        self.load_preset()
        if not self.chk_advanced_mode.IsChecked():
            self.on_simple_profile()
        self.cbo_stereo_format.SetValue("Full SBS")
        self.cbo_stereo_format.Disable()
        ...
        self.update_openxr_state()
        self.update_openxr_slider_labels()
        self.update_ui_mode()
        ...
        self.source_preview_timer = wx.Timer(self)
        self.Bind(wx.EVT_TIMER, self.update_source_preview, self.source_preview_timer)
        self.source_preview_timer.Start(2000)
    ...
    def get_depth_models(self, small_only):
        if small_only:
            ...
            if VideoDepthAnythingStreamingModel.has_checkpoint_file(
                "VDA_Stream_Metric_B"
            ):
    ...
    def on_close(self, event):
        ...
        if hasattr(self, "source_preview_timer"):
    ...
    def update_bind_addr_warning(self, *args, **kwargs):
        ...
        if is_loopback_address(bind_addr):
            self.lbl_bind_addr_warning.SetLabel(
                f"{bind_addr} is only accessible from this PC."
            )
            self.lbl_bind_addr_warning.Show()
        elif not is_private_address(bind_addr):
            self.lbl_bind_addr_warning.SetLabel(
                f"{bind_addr} is not Local Area Network Address."
            )
            self.lbl_bind_addr_warning.Show()
        else:
            self.lbl_bind_addr_warning.SetLabel("")
    ...
    def show_validation_error_message(self, name, min_value, max_value):
        with wx.MessageDialog(
            None,
            message=T("`{}` must be a number {} - {}").format(
    ...
    def parse_args(self):
        ...
        if not validate_number(
            self.cbo_edge_dilation.GetValue(), 0, 20, is_int=True, allow_empty=False
        ):
            self.show_validation_error_message(T("Edge Fix"), 0, 20)
            return None
        if not validate_number(
            self.cbo_edge_dilation_y.GetValue(), 0, 20, is_int=True, allow_empty=True
        ):
            self.show_validation_error_message(T("Edge Fix"), 0, 20)
            return None
        if not validate_number(self.cbo_ema_decay.GetValue(), 0.1, 0.999):
            ...
            return None
        if not validate_number(
            self.cbo_foreground_scale.GetValue(), -3.0, 3.0, allow_empty=False
        ):
            self.show_validation_error_message(T("Foreground Scale"), -3, 3)
            return None
        if not validate_number(
            self.cbo_stream_fps.GetValue(), 1, 240, allow_empty=False
        ):
            self.show_validation_error_message(T("Streaming FPS"), 1, 240)
            return None
        if not validate_number(
            self.cbo_stream_height.GetValue(), 320, 4320, allow_empty=False
        ):
        ...
        for crop_name, crop_value in [
            ...
            if not validate_number(
                str(crop_value), 0, 500, is_int=True, allow_empty=False
            ):
                self.show_validation_error_message(
                    T("Crop") + " " + T(crop_name), 0, 500
                )
                return None
        ...
        if resolution == "Default" or resolution == "":
            ...
            if not validate_number(
                resolution, 192, 8190, is_int=True, allow_empty=False
            ):
                self.show_validation_error_message(
                    T("Depth") + " " + T("Resolution"), 192, 8190
                )
                return
        ...
        if self.cbo_edge_dilation_y.GetValue():
            edge_dilation = [
                int(self.cbo_edge_dilation.GetValue()),
                int(self.cbo_edge_dilation_y.GetValue()),
            ]
        else:
            edge_dilation = int(self.cbo_edge_dilation.GetValue())

        preserve_screen_border = (
            self.chk_preserve_screen_border.IsEnabled()
            and self.chk_preserve_screen_border.IsChecked()
        )
        ...
        openxr = self.chk_openxr.IsChecked()
        if openxr:
            viewer_kwargs = dict(
                openxr=True,
                local_viewer=False,
                ...
                openxr_projection=self.cbo_openxr_projection.GetValue().lower(),
                openxr_distance=self.sld_openxr_distance.GetValue() / 10.0,
                openxr_width=self.sld_openxr_width.GetValue() / 10.0,
                openxr_height=self.sld_openxr_height.GetValue(),
                openxr_curvature=float(self.sld_openxr_curvature.GetValue()),
                openxr_show_fps=self.chk_openxr_show_fps.IsChecked(),
            )
        else:
            viewer_kwargs = dict(
                local_viewer=True,
        ...
        parser.set_defaults(
            ...
            autocrop=(
                self.cbo_autocrop.GetValue() if self.cbo_autocrop.GetValue() else None
            ),
        ...
        args.state["openxr_status_callback"] = OpenXRStatusGUI(self)
        return args

    def on_fps(self, event):
        estimated_fps, screenshot_fps, streaming_fps, screen_size, url = (
            event.GetValue()
        )
        if (
            estimated_fps is not None
            and screenshot_fps is not None
            and streaming_fps is not None
        ):
            self.SetStatusText(
    ...
    def on_openxr_status(self, event):
        if event.state == "Controllers":
            self.txt_openxr_controller.SetValue(
                T("Detected") if "detected" in event.detail.lower() else T("Waiting")
            )
        elif event.state == "Settings":
            values = dict(
                item.split("=", 1) for item in event.detail.split(";") if "=" in item
            )
            if "distance" in values:
                self.sld_openxr_distance.SetValue(round(float(values["distance"]) * 10))
            if "width" in values:
                self.sld_openxr_width.SetValue(round(float(values["width"]) * 10))
            if "curvature" in values:
                self.sld_openxr_curvature.SetValue(round(float(values["curvature"])))
            self.update_openxr_slider_labels()
        else:
            self.txt_openxr_state.SetValue(event.state)
        if event.detail and event.state != "Settings":
            timestamp = time.strftime("%H:%M:%S")
            self.txt_openxr_log.AppendText(
                f"[{timestamp}] {event.state}: {event.detail}\n"
            )
        if event.state == "Paused":
            self.btn_openxr_toggle.SetLabel(T("Resume Display"))
        elif event.state not in {"Stopped", "Error", "Controllers", "Settings"}:
            self.btn_openxr_toggle.SetLabel(T("Pause Display"))
    ...
    def on_click_btn_start(self, event):
        ...
        self.btn_url.Disable()
        self.cbo_stream_height.Disable()
        self.pnl_preset.Hide()
        self.grp_source.Hide()
    ...
    def on_exit_worker(self, result):
        try:
            ...
            wx.MessageBox(
                message, f"{T('Error')}: {e.__class__.__name__}", wx.OK | wx.ICON_ERROR
            )
        ...
        self.cbo_stream_height.Enable()
        self.txt_url.SetValue("")
        self.btn_url.Disable()
        self.pnl_preset.Show()
        self.grp_source.Show()
    ...
    def on_click_openxr_toggle(self, event):
        output = self.args.state.get("openxr_output") if self.args is not None else None
        if output is None:
            enabled = not self.chk_openxr.IsChecked()
            self.chk_openxr.SetValue(enabled)
            self.update_openxr_state()
            return
        enabled = not output.enabled_event.is_set()
        output.set_enabled(enabled)
        self.btn_openxr_toggle.SetLabel(
            T("Pause Display") if enabled else T("Resume Display")
        )

    def on_click_openxr_recenter(self, event):
        output = self.args.state.get("openxr_output") if self.args is not None else None
        if output is not None:
            output.recenter()
        else:
            self.txt_openxr_log.AppendText(
                T("Recenter will be applied after OpenXR starts") + "\n"
            )

    def update_openxr_settings(self, *args, **kwargs):
        self.update_openxr_slider_labels()
        output = self.args.state.get("openxr_output") if self.args is not None else None
        if output is not None:
            output.update_settings(
                projection=self.cbo_openxr_projection.GetValue().lower(),
                distance=self.sld_openxr_distance.GetValue() / 10.0,
                width=self.sld_openxr_width.GetValue() / 10.0,
                height=self.sld_openxr_height.GetValue(),
                curvature=float(self.sld_openxr_curvature.GetValue()),
                show_fps=self.chk_openxr_show_fps.IsChecked(),
            )

    def update_openxr_state(self, *args, **kwargs):
        enabled = self.chk_openxr.IsChecked()
        if enabled:
            self.cbo_stereo_format.SetValue("Full SBS")
        self.btn_openxr_toggle.Show(self.processing and enabled)
        self.btn_openxr_toggle.SetLabel(T("Pause Display"))
        self.update_stereo_format()
        self.GetSizer().Layout()
        self.Fit()

    def update_openxr_slider_labels(self):
        self.txt_openxr_distance_value.SetLabel(
            f"{self.sld_openxr_distance.GetValue() / 10.0:.1f} m"
        )
        self.txt_openxr_width_value.SetLabel(
            f"{self.sld_openxr_width.GetValue() / 10.0:.1f} m"
        )
        self.txt_openxr_curvature_value.SetLabel(
            f"{self.sld_openxr_curvature.GetValue()}°"
        )

    def on_simple_profile(self, *args, **kwargs):
        selection = self.cbo_simple_profile.GetSelection()
        if selection == 0:
            settings = {
                ...
                "resolution": "392",
                "fps": "60",
                "help": T("Prioritizes smooth motion and responsiveness."),
            }
        elif selection == 2:
            settings = {
                ...
                "resolution": "720",
                "fps": "30",
                "help": T("Prioritizes image quality and may reduce frame rate."),
            }
        else:
            settings = {
                "depth_model": "VDA_Stream_S",
                ...
                "resolution": "512",
                "fps": "60",
                "help": T("Balanced is recommended for real-time OpenXR."),
            }
        if self.cbo_depth_model.FindString(settings["depth_model"]) != wx.NOT_FOUND:
            self.cbo_depth_model.SetStringSelection(settings["depth_model"])
        if self.cbo_method.FindString(settings["method"]) != wx.NOT_FOUND:
            self.cbo_method.SetStringSelection(settings["method"])
        self.cbo_resolution.SetValue(settings["resolution"])
        self.cbo_stream_fps.SetValue(settings["fps"])
        self.lbl_simple_profile_help.SetLabel(settings["help"])

    def update_ui_mode(self, *args, **kwargs):
        advanced = self.chk_advanced_mode.IsChecked()
        simple_controls = [
            self.lbl_simple_profile,
            self.cbo_simple_profile,
            self.lbl_simple_profile_help,
        ]
        advanced_stereo_controls = [
            self.lbl_convergence,
            self.cbo_convergence_mode,
            self.cbo_convergence,
            self.lbl_synthetic_view,
            self.cbo_synthetic_view,
            self.lbl_method,
            self.cbo_method,
            self.chk_small_model_only,
            self.lbl_depth_model,
            self.cbo_depth_model,
            self.lbl_resolution,
            self.cbo_resolution,
            self.lbl_foreground_scale,
            self.cbo_foreground_scale,
            self.lbl_edge_dilation,
            self.cbo_edge_dilation,
            self.cbo_edge_dilation_y,
            self.chk_ema_normalize,
            self.cbo_ema_decay,
            self.chk_preserve_screen_border,
            self.lbl_stereo_format,
            self.cbo_stereo_format,
            self.lbl_anaglyph_method,
            self.cbo_anaglyph_method,
            self.lbl_format_device,
        ]
        openxr_advanced_controls = [
            self.lbl_openxr_runtime,
            self.txt_openxr_runtime,
            self.lbl_openxr_height,
            self.sld_openxr_height,
            self.txt_openxr_log,
        ]
        for control in simple_controls:
            control.Show(not advanced)
        for control in advanced_stereo_controls + openxr_advanced_controls:
            control.Show(advanced)
        self.grp_network.Hide()
        self.grp_processor.Show(advanced)
        self.GetSizer().Layout()
        self.Fit()
    ...
    def save_preset(self, name=None):
        ...
        for control in self.get_editable_comboboxes():
            persistent_manager_register(
                manager, control, EditableComboBoxPersistentHandler
            )
        ...
        try:
            ...
            if divergence > max_divergence:
                self.lbl_divergence_warning.SetLabel(
                    f"{divergence}: "
                    + T("Out of range of training data")
                    + f": {method}, {synthetic_view}"
                )
    ...
    def update_source_selector(self, *args, **kwargs):
        source_type = self.cbo_source_type.GetSelection()
        previous_value = (
            self.cbo_source_choice.GetClientData(self.cbo_source_choice.GetSelection())
            if self.cbo_source_choice.GetSelection() != wx.NOT_FOUND
            else None
        )
        self.cbo_source_choice.Clear()
        if source_type == 0:
            self.lbl_source_choice.SetLabel(T("Screen"))
            for index, (width, height) in enumerate(get_monitor_size_list()):
                self.cbo_source_choice.Append(
                    f"{T('Screen')} {index + 1} — {width} × {height}",
                    ("monitor", index),
                )
        else:
            self.lbl_source_choice.SetLabel(T("Window"))
            for window_name in enum_window_names():
                self.cbo_source_choice.Append(window_name, ("window", window_name))
        selection = 0
        for index in range(self.cbo_source_choice.GetCount()):
            if self.cbo_source_choice.GetClientData(index) == previous_value:
                selection = index
                break
        if self.cbo_source_choice.GetCount():
            self.cbo_source_choice.SetSelection(selection)
        self.on_source_choice()

    def on_source_choice(self, *args, **kwargs):
        selection = self.cbo_source_choice.GetSelection()
        if selection == wx.NOT_FOUND:
            return
        source_type, source_value = self.cbo_source_choice.GetClientData(selection)
        if source_type == "monitor":
            monitor_index = int(source_value)
            if monitor_index < self.cbo_monitor_index.GetCount():
                self.cbo_monitor_index.SetSelection(monitor_index)
            self.cbo_window_name.SetValue("")
        else:
            self.cbo_monitor_index.SetSelection(0)
            self.cbo_window_name.SetValue(source_value)
        if self.cbo_screenshot.GetValue() not in {"wc_cuda", "wc_mp", "mss"}:
            for backend in ("wc_cuda", "wc_mp", "mss"):
                index = self.cbo_screenshot.FindString(backend)
                if index != wx.NOT_FOUND:
                    self.cbo_screenshot.SetSelection(index)
                    break
        self.update_source_preview()

    def update_source_preview(self, *args, **kwargs):
        if self.processing:
            return
        selection = self.cbo_source_choice.GetSelection()
        if selection == wx.NOT_FOUND:
        ...
        try:
            source_type, source_value = self.cbo_source_choice.GetClientData(selection)
            if source_type == "monitor":
                bbox = self._monitor_rect(int(source_value))
            else:
                rect = get_window_rect_by_title(source_value)
                if rect is None:
                    raise RuntimeError(T("The selected window is no longer available."))
                bbox = (
                    rect["left"],
                    rect["top"],
            ...
            preview = ImageGrab.grab(
                bbox=bbox, include_layered_windows=True, all_screens=True
            )
            self._set_source_preview(preview)
        except Exception as error:
            width, height = self.bmp_source_preview.GetSize()
            preview = Image.new("RGB", (max(width, 1), max(height, 1)), "#20242a")
            draw = ImageDraw.Draw(preview)
            draw.text((12, 12), f"{T('Preview unavailable')}\n{error}", fill="#f0f0f0")
            self._set_source_preview(preview)

    @staticmethod
    def _monitor_rect(monitor_index):
        if sys.platform == "win32":
            import win32api
        ...
        monitor = mss.mss().monitors[monitor_index + 1]
        return (
            monitor["left"],
            monitor["top"],
    ...
    def _set_source_preview(self, image):
        target_width, target_height = self.source_preview_size
        target_width = max(target_width, 1)
        target_height = max(target_height, 1)
        image = image.convert("RGB")
        image.thumbnail((target_width, target_height), Image.Resampling.LANCZOS)
        canvas = Image.new("RGB", (target_width, target_height), "#101215")
        offset = (
            (target_width - image.width) // 2,
            (target_height - image.height) // 2,
        )
        canvas.paste(image, offset)
        wx_image = wx.Image(target_width, target_height)
        wx_image.SetData(canvas.tobytes())
        self.bmp_source_preview.SetBitmap(wx.Bitmap(wx_image))
...
for locale_name in sorted(LOCALES):
    if not any(LOCALES[locale_name] is LOCALES[name] for name in LOCAL_LIST):
        LOCAL_LIST.append(locale_name)
...
def main():
    ...
    parser.add_argument(
        "--lang", type=str, choices=list(LOCALES.keys()), help="translation language"
    )
    args = parser.parse_args()
    if args.lang:
