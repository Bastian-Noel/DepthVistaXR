...
def get_monitor_rect(monitor_index):
    if sys.platform == "win32":
        ...
        left, top, right, bottom = win32api.EnumDisplayMonitors()[monitor_index][2]
...
def create_parser():
    ...
    parser.add_argument("--openxr", action="store_true", help="Use OpenXR output instead of Web Streaming")
    parser.add_argument("--openxr-projection", choices=["flat", "curved"], default="curved")
    parser.add_argument("--openxr-distance", type=float, default=2.0, help="Virtual screen distance in meters")
    parser.add_argument("--openxr-width", type=float, default=3.0, help="Virtual screen width in meters")
    parser.add_argument("--openxr-height", type=float, default=0.0, help="Virtual screen height in meters; 0 is automatic")
    parser.add_argument("--openxr-curvature", type=float, default=30.0, help="Virtual screen curvature in degrees")
    parser.add_argument("--openxr-show-fps", action="store_true", help="Show FPS in the top-right corner")
    parser.add_argument(
        "--openxr-control-profile",
        choices=["cinema", "desktop"],
        default="cinema",
        help="OpenXR controller mapping",
    )
    parser.add_argument(
        "--openxr-pointer",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable controller ray pointer and desktop mouse control",
    )
    parser.add_argument(
        "--openxr-right-click",
        action=argparse.BooleanOptionalAction,
...
def set_state_args(args, args_lock=None, stop_event=None, fps_event=None, depth_model=None):
    ...
    args.state.setdefault("openxr_status_callback", None)
    args.state.setdefault("openxr_output", None)
...
def iw3_desktop_main(args, init_wxapp=True):
    ...
    if args.openxr:
        ...
        args.tb = False
        args.half_tb = False
        args.cross_eyed = False
        args.rgbd = False
        args.half_rgbd = False
        args.anaglyph = None
    elif not any([args.full_sbs, args.tb, args.half_tb,
    ...
    if not args.local_viewer and not args.openxr:
    ...
    if args.openxr:
        from .openxr_output import OpenXROutput
        if args.window_name:
            pointer_window = get_window_rect_by_title(args.window_name)
            pointer_rect = (
                pointer_window["left"] + args.crop_left,
                pointer_window["top"] + args.crop_top,
                pointer_window["left"] + pointer_window["width"] - args.crop_right,
                pointer_window["top"] + pointer_window["height"] - args.crop_bottom,
            )
        else:
            pointer_rect = get_monitor_rect(args.monitor_index)
        server = OpenXROutput(
            projection=args.openxr_projection,
            distance=args.openxr_distance,
            width=args.openxr_width,
            height=args.openxr_height,
            curvature=args.openxr_curvature,
            show_fps=args.openxr_show_fps,
            control_profile=args.openxr_control_profile,
    ...
    try:
        ...
        while True:
            ...
            if args.local_viewer or args.openxr:
                ...
                if server.is_closed():
                    if args.openxr and server.get_error() is not None:
                        raise RuntimeError(f"OpenXR output stopped: {server.get_error()}")
                    break
